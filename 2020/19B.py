    ...
    # mid level, has to match both parts put together
        ...
        if len(rule) == 1:
            ...
            for split_point in range(1, len(message)):
                if (cache_matches(message[:split_point], rule[0], depth)
                    and cache_matches(message[split_point:], rule[1], depth)):
                    return True
            return False
        else:
            for split_point_1 in range(1, len(message) - 1):
                for split_point_2 in range(split_point_1, len(message)):
                    if (cache_matches(message[:split_point_1], rule[0], depth)
                        and cache_matches(message[split_point_1:split_point_2], rule[1], depth)
                        and cache_matches(message[split_point_2:], rule[2], depth)):
                        return True
            return False
